...
- `js/llm.js` — the typed/model path: HF OAuth (`oauthLoginUrl` / `oauthHandleRedirectIfPresent`, viewer token) + Inference Providers (`InferenceClient.chatCompletion`), called from the browser so inference bills to the viewer. `generateOverlay(task)` uses `SCHEMA.md` as the system prompt and the `demo.js` overlays as few-shot, with guided JSON + a corrective retry. HF libs load lazily from a pinned CDN and fail soft.
...
**Flow:** load → seed `data-theme` → `boot()` → `renderSuggestions` + `renderNav` + `initAuth` → `handleHash()` resolves the hash against `effective(BASE)` → `renderView()` tiles the tesserae. Driving it: a typed task (signed in) → `generateOverlay` → `mosaic:apply`; an example → `applyOverlay`; the Composer → validate → `mosaic:apply`. Each sets `STATE.overlay` and re-routes; the command-bar controls are unaffected.
