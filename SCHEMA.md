...
MosAIc has a built-in **bring-your-own** model path: signed in with Hugging Face, a typed task is sent to a model on [Inference Providers](https://huggingface.co/docs/inference-providers) billed to the viewer. This schema is the system prompt, the example overlays are the few-shot, and the validated result feeds the `mosaic:apply` path (see `js/llm.js`). Successive tasks are given the **current surface** and emit a *patch* against it — `composeOverlay()` folds each into the accumulated overlay by the same id-merge rules, so the mosaic **evolves** turn over turn instead of regenerating; `Reset` returns to base. No model *ships* with MosAIc and there's no shared key — the Composer drives the same path with hand-edited JSON, and forks can point `js/llm.js` at any chat-completion endpoint.
