...
**OKF viewer shipped (v1.2):** MosAIc opens a Google **Open Knowledge Format** bundle (a folder of markdown + YAML frontmatter) and renders it as a calm reading surface — one view per concept `type`, cross-links that navigate in-app, and a **provenance strip** (`sourced/unsourced`, source link, freshness) since OKF's frontmatter carries no trust field. Pure core `js/okf.js` (tested); loader `js/okf-load.js`; built-in sample under `samples/`; brief in `OKF-VIEWER-BRIEF.md`. Deferred: **search + type/tag filter** (table-stakes polish); `.zip` + URL/GitHub loaders (folder + sample ship now); concept-level scroll on same-view cross-links.
